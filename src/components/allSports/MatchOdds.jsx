...
const MatchOdds = ({
	...
	const renderOddBox = (key, item, className, runner, orderType) => {
		...
		return (
			<div
				key={key}
				className={`market-odd-box ${showOdds < 4 ? ' !w-[58px] lg:!w-[85px]' : '!w-[30px] sm:!w-[58px] lg:!w-[85px]'}  min-h-[35px] min-sm:h-[45px] text-center flex flex-col items-center justify-center gap-1 ${className} ${item.changed ? "blink" : ""
					} ${key != 2 &&
					(sportId == "7" || sportId == "4339") &&
					"!hidden md:!flex"
					}`}
